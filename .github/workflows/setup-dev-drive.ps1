...
#
# When using self-hosted runners, the underlying infrastructure may not
...
# DevDrive, as that also requires Hyper-V. Self-hosted runners may use `D:`
# already, so we must check if it's self-hosted first, and we use `V:` as the
# target instead.


if ($env:RUNNER_ENVIRONMENT -eq "self-hosted") {
    Write-Output "Self-hosted runner detected, setting up custom dev drive..."
...
# Prepare Cargo and rustup homes on the dev drive before setup-rust installs the toolchain.
