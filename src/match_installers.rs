...
pub fn match_installers(
    ...
    previous_installers
        ...
        .map(|mut previous_installer| {
            ...
            // executable candidates, so prefer an architecture match with the same file extension.
            ...
            let has_matching_nested_architecture = previous_installer
                .nested_installer_type
                .is_some()
                && new_installers.iter().any(|new_installer| {
                    new_installer.architecture == previous_installer.architecture
                        && ValidFileExtensions::from_url(&new_installer.url) == previous_extension
                });
...
mod tests {
    ...
    #[test]
    fn matches_shared_portable_zip_alongside_msixbundle() {
        let new_zip = Installer {
            architecture: Architecture::Neutral,
            r#type: Some(InstallerType::Zip),
            url: DecodedUrl::from_str("https://example.com/NanaBox_2.0_Binaries.zip").unwrap(),
            ..Installer::default()
        };
        let mut previous = Vec::new();
        let mut new = vec![new_zip.clone()];
        let mut expected = HashMap::new();
        for architecture in [Architecture::X64, Architecture::Arm64] {
            let previous_zip = Installer {
                architecture,
                nested_installer_type: Some(NestedInstallerType::Portable),
                url: DecodedUrl::from_str("https://example.com/NanaBox_1.0_Binaries.zip").unwrap(),
                ..new_zip.clone()
            };
            let previous_msix = Installer {
                architecture,
                r#type: Some(InstallerType::Msix),
                url: DecodedUrl::from_str("https://example.com/NanaBox_1.0.msixbundle").unwrap(),
                ..Installer::default()
            };
            let new_msix = Installer {
                url: DecodedUrl::from_str("https://example.com/NanaBox_2.0.msixbundle").unwrap(),
                ..previous_msix.clone()
            };
            previous.extend([previous_zip.clone(), previous_msix.clone()]);
            new.push(new_msix.clone());
            expected.extend([(previous_zip, new_zip.clone()), (previous_msix, new_msix)]);
        }

        let matched = match_installers(&previous, &new);
        assert_eq!(matched, expected);
        assert!(unmatched_installers(&matched, &new).is_empty());
    }
