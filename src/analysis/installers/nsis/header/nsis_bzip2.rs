/*
 * Adapted from https://github.com/ATRAPSLLC/nsis-rs/blob/main/src/decompress/bzip2.rs
 *
...
impl<R: Read> Decoder<R> {
    pub fn new(
        ...
        Ok(Self {
            inner: reader,
            decompressed: Cursor::new(
                decompress(&compressed, max_output).map_err(|error| {
        ...
        loop {
            if self.live >= count {
                ...
                return i32::try_from(value)
                    .map_err(|_| eyre!("NSIS bzip2 bit value out of range"));
            }

            let byte = self
                .data
                .get(self.position)
                .ok_or_else(|| eyre!("Unexpected end of NSIS bzip2 input"))?;
...
pub fn decompress(compressed: &[u8], max_output: usize) -> Result<Vec<u8>> {
    match decompress_layout(compressed, max_output, BlockLayout::Modern) {
        Ok(output) => Ok(output),
        Err(modern_error) => {
            decompress_layout(compressed, max_output, BlockLayout::Nsis1).map_err(|_| modern_error)
        }
    }
}

fn decompress_layout(compressed: &[u8], max_output: usize, layout: BlockLayout) -> Result<Vec<u8>> {
    match panic::catch_unwind(AssertUnwindSafe(|| {
        decompress_inner(compressed, max_output, layout)
    })) {
        Ok(result) => result,
        Err(_) => Err(eyre!("NSIS bzip2 decoder panicked on malformed input")),
    }
}

fn decompress_inner(compressed: &[u8], max_output: usize, layout: BlockLayout) -> Result<Vec<u8>> {
    if compressed.is_empty() {
        return Err(eyre!("Empty NSIS bzip2 input"));
    }
    ...
    loop {
        match reader.get_u8()? {
            0x17 => break,
            0x31 => decompress_block(&mut reader, &mut output, max_output, layout)?,
            header => {
                return Err(eyre!("Invalid NSIS bzip2 block header 0x{header:02X}",));
            }
        }
...
fn decompress_block(
    ...
    if layout == BlockLayout::Nsis1 && reader.get_bit()? == 1 {
        return Err(eyre!("NSIS bzip2 randomised blocks are not supported"));
    }
    ...
    if !(0..=10 + BLOCK_SIZE_I32).contains(&orig_ptr) {
        return Err(eyre!("NSIS bzip2 origPtr out of range: {orig_ptr}",));
    }
    ...
    for (symbol, used) in in_use.into_iter().enumerate() {
        if used {
            seq_to_unseq[n_in_use] =
                u8::try_from(symbol).map_err(|_| eyre!("NSIS bzip2 symbol out of range"))?;
            n_in_use += 1;
        }
    }
    if n_in_use == 0 {
        return Err(eyre!("NSIS bzip2 block has no symbols in use"));
    }

    let alpha_size = n_in_use + 2;
    let n_groups = reader.get_bits(3)?;
    if !(2..=6).contains(&n_groups) {
        return Err(eyre!("NSIS bzip2 nGroups out of range: {n_groups}",));
    }
    let n_groups =
        usize::try_from(n_groups).map_err(|_| eyre!("NSIS bzip2 nGroups out of range"))?;

    let n_selectors = reader.get_bits(15)?;
    if !(1..=BZ_MAX_SELECTORS_I32).contains(&n_selectors) {
        return Err(eyre!("NSIS bzip2 nSelectors out of range: {n_selectors}",));
    }
    let n_selectors =
        usize::try_from(n_selectors).map_err(|_| eyre!("NSIS bzip2 nSelectors out of range"))?;
    ...
    for selector in &mut selector_mtf {
        ...
        loop {
            ...
            value += 1;
            if value >= n_groups {
                return Err(eyre!("NSIS bzip2 selector MTF value out of range"));
            }
        }
        *selector = u8::try_from(value).map_err(|_| eyre!("NSIS bzip2 selector out of range"))?;
    }
    ...
    for (value, position) in positions.iter_mut().enumerate().take(n_groups) {
        *position =
            u8::try_from(value).map_err(|_| eyre!("NSIS bzip2 selector position out of range"))?;
    }
        ...
        for slot in table.iter_mut().take(alpha_size) {
            loop {
                if !(1..=20).contains(&current) {
                    return Err(eyre!("NSIS bzip2 code length out of range: {current}",));
                }
                if reader.get_bit()? == 0 {
                    break;
                }
            ...
            *slot =
                u8::try_from(current).map_err(|_| eyre!("NSIS bzip2 code length out of range"))?;
        }
    }
    ...
    let mut huffman = HuffmanTables {
        ...
        n_selectors: i32::try_from(n_selectors)
            .map_err(|_| eyre!("NSIS bzip2 nSelectors out of range"))?,
        group_no: -1,
        group_pos: 0,
    };
    ...
    let eob =
        i32::try_from(n_in_use + 1).map_err(|_| eyre!("NSIS bzip2 EOB symbol out of range"))?;
    ...
    for ii in (0..(256 / MTFL_SIZE)).rev() {
        for jj in (0..MTFL_SIZE).rev() {
            mtfa[kk] = u8::try_from(ii * MTFL_SIZE + jj)
                .map_err(|_| eyre!("NSIS bzip2 MTF value out of range"))?;
    ...
    loop {
        ...
        if next_sym == BZ_RUNA || next_sym == BZ_RUNB {
            ...
            let es =
                usize::try_from(es).map_err(|_| eyre!("NSIS bzip2 run length out of range"))?;
            if nblock + es > BLOCK_SIZE {
                return Err(eyre!("NSIS bzip2 block overflow during RLE"));
            }
            for _ in 0..es {
        ...
        if nblock >= BLOCK_SIZE {
            return Err(eyre!("NSIS bzip2 block overflow"));
        }
    ...
    let orig_ptr =
        usize::try_from(orig_ptr).map_err(|_| eyre!("NSIS bzip2 origPtr out of range"))?;
    if orig_ptr >= nblock {
        return Err(eyre!(
    ...
    if cftab[256] != i32::try_from(nblock).map_err(|_| eyre!("NSIS bzip2 block is too large"))? {
        return Err(eyre!("NSIS bzip2 cftab consistency check failed"));
    }

    for i in 0..nblock {
        let symbol = (tt[i] & 0xff) as usize;
        let destination = usize::try_from(cftab[symbol])
            .map_err(|_| eyre!("NSIS bzip2 cftab index out of range"))?;
        tt[destination] |=
            u32::try_from(i).map_err(|_| eyre!("NSIS bzip2 block index out of range"))? << 8;
        cftab[symbol] += 1;
    }
...
fn get_mtf_val(reader: &mut BitReader<'_>, huffman: &mut HuffmanTables) -> Result<i32> {
    if huffman.group_pos == 0 {
        ...
        if huffman.group_no >= huffman.n_selectors {
            return Err(eyre!("NSIS bzip2 ran out of selectors"));
        }
    ...
    let group_no = usize::try_from(huffman.group_no)
        .map_err(|_| eyre!("NSIS bzip2 group index out of range"))?;
    ...
    loop {
        if zn > 20 {
            return Err(eyre!("NSIS bzip2 Huffman code length exceeds 20"));
        }
        let zn_index =
            usize::try_from(zn).map_err(|_| eyre!("NSIS bzip2 code length out of range"))?;
    ...
    let zn_index = usize::try_from(zn).map_err(|_| eyre!("NSIS bzip2 code length out of range"))?;
    let index = zvec - huffman.base[selected][zn_index];
    if !(0..BZ_MAX_ALPHA_SIZE_I32).contains(&index) {
        return Err(eyre!("NSIS bzip2 Huffman index out of range"));
    }
    Ok(huffman.perm[selected]
        [usize::try_from(index).map_err(|_| eyre!("NSIS bzip2 Huffman index out of range"))?])
}
    ...
    while nblock_used <= nblock {
        ...
        if state_out_len > 0 {
            let emit_count = usize::try_from(state_out_len)
                .map_err(|_| eyre!("NSIS bzip2 output run length out of range"))?
...
fn bz_get_fast(tt: &[u32], t_pos: &mut u32) -> Result<u8> {
    let entry = *tt
        .get(*t_pos as usize)
        .ok_or_else(|| eyre!("NSIS bzip2 BWT position out of range"))?;
    *t_pos = entry >> 8;
    Ok((entry & 0xff) as u8)
}
