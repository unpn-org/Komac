...
use super::error::{InvalidArgument, to_napi_error};
...
pub fn parse_yaml(
    ...
    let failsafe = match schema.as_deref() {
        ...
        Some("failsafe") => true,
        Some(schema) => {
            return Err(InvalidArgument(format!("Invalid YAML schema {schema:?}")).into());
        }
    };
    ...
    let json = serde_json::to_string(&value).map_err(|error| to_napi_error(error.into()))?;
